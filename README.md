...
function solution(n, ladder) {
    const players = [...new Array(n)].map((_, index) => index);
    const order = [];

    function getStartLine(position) {
    ...
    for (const player of players) {
        let position = player;
        let startLine = getStartLine(position);
        for (let y = 0; y < ladder.length; y++) {
            const row = ladder[y];
            const isTrue = row[startLine] === 1;
            const isPrevTrue = startLine - 1 === position && row[startLine - 1] === 1;
            const isNextTrue = startLine + 1 === position && row[startLine + 1] === 1;
            if (isTrue) {
                if (position === startLine) {
                    position += 1;
                    if (startLine < row[startLine].length - 1) {
                        startLine += 1;
                    }
                } else if (position - 1 === startLine) {
                    position -= 1;
                    if (startLine > 0) {
                        startLine -= 1;
                    }
